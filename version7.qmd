---
title: "Bank Customer Attrition Strategic Review"
subtitle: "Leveraging Machine Learning Insights to Identify Key Factors Associated with Customer Attrition"
...
The dataset utilized in this analysis comes from Kaggle: https://www.kaggle.com/datasets/sakshigoyal7/credit-card-customers

It consists of 10,000 rows of anonymized customer data for current and former customers, and includes both demographic data (gender, age, marital status, income, education) and account data (credit limit, revolving balance, transaction information, communication with bank information, etc.). Customer churn (former bank customers) account for about 16% of the dataset. 

The goal here is to leverage machine learning functionality within R to better understand the key factors for those customers who have already left the bank in order to better identify current customers at risk.
:::
...
This final section uses survival analysis to map the customer lifecycle. Sudden drops in this curve indicate risk milestones - specific anniversary dates where customers are statistically most likely to reconsider their relationship with the bank. 
...
The analysis of over 10,000 customer records confirms that attrition in the consumer credit card division is rarely a sudden event, but rather a slow behavioral drift. By utilizing a random forest model, the strongest predictors of churn that have been predicted are not demographic markers like age or income, but rather a sharp decline in "transaction velocity and average utilization. When a customer's total transaction count drops or they cease maintaining a revolving balance, they may be signaling their intent to depart months before the account is actually closed. 

Additionally, survival analysis identified a critical "tenure risk" at the 36-month milestone. This suggests that as customers reach their third anniversary with the bank, the initial product appeal or promotional incentives may be losing their efficacy. To mitigate this, a structural shift in the Customer Lifecycle Management process might be valuable - implementing automated stay-active incentives and product reviews specifically timed for this 3-year window.

This project only scratches the surface of how machine learing and logistic regression can help solve these types of challenges in the credit and risk markets.